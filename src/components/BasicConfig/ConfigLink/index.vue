...
export default {
    ...
    linkText() {
      ...
      if (clickType && notEmptyObj) {
        ...
        if (arr.length > 0) {
          let subName =
            clickType === 10
              ? data.orderService[arr[0].linkTextAttr]
              : data[arr[0].linkTextAttr];
  ...
  methods: {
    ...
    setInitData() {
      ...
      let cloneData = hasValue
        ? this.cloneObj(this.linkObj)
        : this.cloneObj(emptyData);
