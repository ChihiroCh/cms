...
export function dynamicComponents() {
  ...
  dynamic.keys().map(item => {
    ...
    components[name] = dynamic(item).default;
  });
  return components;
}
...
export class Messager {
  ...
  // 发送数据
  emit(type, value) {
    const win = document.getElementById('previewIframe').contentWindow;
    win.postMessage({ type, value }, this.targetOrigin);
    return this;
  }
  destory() {
